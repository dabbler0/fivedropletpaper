...
A key motivation for Droplet is to allow students edit any program with blocks. Droplet is built as a block editor framework that supports multiple languages.
...
Adding two-dimensional editing to Droplet presents an obstacle. Two-dimensional editing helps tinkerability, but is inconsistent with the linear nature of text programs.

Droplet's solution to this dilemma is to allow the construction of "floating" blocks outside of the main text program which are not executed when the program is run. Droplet surrounds floating blocks with the comment symbol and a dotted line to indicate that they will not be run (figure \ref{floating}). Droplet can display a "play" button (as seen in figure \ref{floating}) to allow students to experiement with running individual stacks. However, the stacks are not included in the main program.
...
Especially in untyped languages like JavaScript, it is easy to accidentally drop Droplet blocks into the wrong socket, and so it is important for users to be able to recover from that action. Other major block languages do not have this problem because sockets with information like variable names or strings are not usually drop targets for other blocks. Scratch supports single-level undo, but neither Scratch and Blockly supports a full undo stack. In contrast, Droplet needs robust support for recovery from mistakes. The author found two strategies for recovery: to arrange the removal of a block from a socket to have a predictable result, and to implement a full undo stack.
