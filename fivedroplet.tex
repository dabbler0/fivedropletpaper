...
\title{Integrating Droplet into Applab -- Five Observations and Improvements}
...
Droplet \cite{Droplet} is a new programming editor that allows dual-mode editing in blocks and text for any text program. This paper presents observations and improvements to Droplet based on integrating Droplet into Applab, Code.org's JavaScript sandbox learning environment. While integrating Droplet into Applab the author encountered five problems with Droplet affecting its usability.
...
\subsection{Motivation}
Applab had existing support for live errors and warnings and debugging breakpoints in text mode. Educational research shows \cite{DebugResearch} that breakpoints are important to students' education. Because Droplet blocks have a one-to-one relationship with text code, adding breakpoint and live line-annotation support to Droplet could easily take advantage of Applab's existing debugging infrastructure.

\subsection{Related Work}
This is a unique opportunity for Droplet, because it has a one-to-one relationship totext code. Neither Scratch nor Blockly have support for line annotations or line breakpoints. Line breakpoints and live annotations, however, have been a part of professional editors since (FIND) and are included in popular editors like (FIND) and (FIND). Research (FIND) shows that breakpoints and debugging are important to education in text languages, and other research (FIND) shows that live error messages help prevent syntax errors before they happen.

\subsection{Solution}
Droplet now supports breakpoints and annotations in the gutter the same way major text editors do. Droplet mimics Ace editor's API to allow Applab and other embedders to easily convert their existing debugging infrastructure from Ace editor to Droplet.

\section{Dropdowns in Text-Generated Blocks}

\subsection{Motivation}
Because all Droplet blocks are generated from text, Droplet did not have good support for dropdowns from sockets, which other major block languages do.
...
\subsection{Solution}
Droplet now allows the embedding application layer to specify dropdowns for arguments to specific functions in JavaScript and CoffeeScript mode. Dropdowns can display arbitrary HTML for each option, allowing Applab to display images for colors or background image options. For dropdowns like variables, the options list can be dynamically generated.
...
\subsection{Solution}
Droplet now remembers the old values of a socket when a block is dropped into it, so that when the block is dragged back out the socket is repopulated with the old value. This prevents students from losing work by accidentally dropping blocks into the wrong socket.
...
\subsection{Motivation}
Research shows \cite{FloatingResearch} that showing pieces of code side-by-side two-dimensionally, the way that Scratch and Blockly support, is useful educationally. This kind of view, however, is unfriendly to the linear layout of text code. Droplet needed a way to support assembling and viewing code side-by-side while retaining a one-to-one relationship to text code.
...
\subsection{Solution}
Droplet now allows floating blocks to be assembled, but surrounds them with a dotted line and the language's block comment symbol to indicate that they are not going to be run. The surrounding dotted line can be grabbed to move the entire block or replace it into the main document to "uncomment" it.
